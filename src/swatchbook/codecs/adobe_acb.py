...
class adobe_acb(SBCodec):
	...
	def read(swatchbook,file):
		...
		if length > 0:
			swatchbook.info.title = decode_str(struct.unpack(str(length*2)+'s',file.read(length*2))[0].decode('utf_16_be'))
		length = struct.unpack('>L',file.read(4))[0]
		if length > 0:
			prefix = decode_str(struct.unpack(str(length*2)+'s',file.read(length*2))[0].decode('utf_16_be'))
		else:
			prefix = ''
		length = struct.unpack('>L',file.read(4))[0]
		if length > 0:
			suffix = decode_str(struct.unpack(str(length*2)+'s',file.read(length*2))[0].decode('utf_16_be'))
		else:
			suffix = ''
		length = struct.unpack('>L',file.read(4))[0]
		if length > 0:
			swatchbook.info.rights = decode_str(struct.unpack(str(length*2)+'s',file.read(length*2))[0].decode('utf_16_be'))
		...
		for i in range(nbcolors):
			...
			length = struct.unpack('>L',file.read(4))[0]
			if length > 0:
				item.info.title = prefix+decode_str(struct.unpack(str(length*2)+'s',file.read(length*2))[0].decode('utf_16_be'))+suffix
			...
			swatchbook.book.items.append(Swatch(id))
		if file.read(4):
			if struct.unpack('>4s',file.read(4))[0] == b'spot':
