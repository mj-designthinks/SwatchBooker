...
class icc_nmcl(SBCodec):
	...
	def read(swatchbook,file):
		...
		prefix = prefix.split(b'\x00', 1)[0].decode('latin_1')
		suffix = suffix.split(b'\x00', 1)[0].decode('latin_1')
		for i in range(n):
			item = Color(swatchbook)
			# This is supposed to be coded in plain ascii but X-Rite Pantone NCPs use Latin 1
			id = prefix+struct.unpack('>32s',file.read(32))[0].split(b'\x00', 1)[0].decode('latin_1')+suffix
